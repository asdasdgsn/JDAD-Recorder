...
本文件记录不同开发阶段的验证。以下较早的权限故障描述属于历史状态；截至 Mac 0.4.4，已通过稳定证书签名、启动新版、查看缩略图并观察到实际录制状态，尚未完成所有真实设备场景验收。当前功能与平台说明以 [README](README.md) 为准。
...
- PASS：`./scripts/build-app.sh`，release 编译、标准 `.app` 目录、应用图标、证书签名及签名校验（0.3.1 起拒绝 ad-hoc）。
...
系统在获取屏幕/窗口列表时返回 TCC 拒绝，未自动修改用户的权限。已向用户说明开启录屏权限的路径，该历史阻塞后续已解除；此处保留早期验收背景。
...
这是可运行的本机测试版，不是已经完成真实录屏验收、正式签名和团队分发的发布版本。工程素材保存在本地；源码仓库同步不包含用户录制素材。
