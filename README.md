...
- 打开 `dist/v0.4.4/JDAD Recorder.app`。首次录制在系统设置 → 隐私与安全性 → 屏幕与系统音频录制中开启 Demo Recorder，然后重新打开应用或刷新列表。
...
- 成功开始录制后，在录制来源所在屏幕顶部居中显示“停止录制”与已录时长。
- 悬浮面板不抢占当前演示窗口焦点，切换应用后仍显示；点击后收起面板、保存并显示剪辑界面。
- 窗口录制按窗口覆盖面积选择显示器；屏幕变化时重新定位到可用屏幕。
- 屏幕/区域捕获沿用排除本应用的过滤器，窗口捕获只包含所选外部窗口，因此悬浮控件不属于录制内容。
- 菜单栏和应用内原有停止入口仍保留；异常结束与退出复用同一停止清理流程。
- 验证：44 项现有测试通过，生产构建与稳定证书签名通过。已启动新版本，实际录制状态可见；未自动点击停止当前用户录制。
